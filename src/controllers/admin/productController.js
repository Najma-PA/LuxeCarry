  ...
  try {
    ...
    if (req.body.variantType) {
      ...
      variants = types.map((type, i) => {
        ...
        if (ids[i]) {
          variantObj._id = ids[i];
        }
        return variantObj;
      });
    }

    await productService.updateProduct(req.params.id, req.body, req.files, variants);
...
exports.restoreProduct = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: 'Server error during restoration: ' + err.message });
  }
};
