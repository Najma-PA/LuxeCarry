...
exports.loginUser = async (req, res) => {
  try {
    ...
    if (!user)
      return res.render('user/login', { error: 'Invalid credentials', formData: req.body });
...
exports.updateProfilePic = async (req, res) => {
  try {
    ...
    const user = await userService.findUserById(userId);
    const oldPublicId = user.profilePic ? user.profilePic.public_id : null;

    // Save using the new service helper
    const imagePath = await userService.saveProfilePic(file, oldPublicId);
    debugLog('Saved to: ' + JSON.stringify(imagePath));
    ...
    res
      .status(500)
      .json({ success: false, message: error.message || 'Failed to update profile picture' });
  }
};
...
exports.removeProfilePic = async (req, res) => {
  try {
    ...
    debugLog(
      'Removing profile picture for user: ' +
        (req.session.user ? req.session.user.id : 'No session')
    );
    ...
    const userId = req.session.user.id;
    const updatedUser = await userService.removeProfilePic(userId);
