...
const getCart = async (userId) => {
  ...
  cart.items.forEach((item) => {
    ...
    const categoryOffer =
      item.product.category && item.product.category.offer ? item.product.category.offer : 0;
...
*/
  ...
  const existing = cart.items.find(
    (i) => i.product.equals(productId) && (variantId ? i.variant?.equals(variantId) : !i.variant)
  );
  ...
  if (existing) {
    ...
    if (newQty > availableStock) {
      throw new Error(
        `Only ${availableStock} units available in stock. You already have ${existing.quantity} in cart.`
      );
    }
...
const updateQuantity = async (userId, itemId, change) => {
  ...
  const updatedItem =
    item.quantity > 0
      ? updatedCart.items.find((i) => i._id.toString() === itemId.toString())
      : null;
